...
} from '@/components/form/formIconXml';
...
export default function FormUploadField({
  ...
  const dropzone = (
    <Pressable
      ...
      className={`w-full items-center justify-center rounded-lg border-[1.2px] border-dashed p-4 ${
        isAccent ? 'h-[95px] border-primary/30 bg-primary/10' : 'h-[91px] border-slate-200 bg-white'
      }`}>
      <View className="items-center gap-1">
        <View className={`overflow-hidden ${isAccent ? 'size-[26px]' : 'size-7'}`}>
          <SvgXml
            xml={isAccent ? CLOUD_UPLOAD_GREEN_ICON_XML : CLOUD_UPLOAD_ICON_XML}
            width={isAccent ? 26 : 28}
            height={isAccent ? 26 : 28}
          />
        </View>
  ...
  if (!panelTitle && !panelHint) {
    return (
      <FormField label={label} required={required} optionalHint={optionalHint} error={error}>
        {dropzone}
      </FormField>
    );
  }
