...
import MemberQrBottomSheet, { type MemberQrBottomSheetRef } from './MemberQrBottomSheet';
...
export default function MemberDetailsScreen() {
  ...
  const member = useMemo(() => state.items.find((item) => item.id === id), [id, state.items]);
    ...
    return (
      <ScreenSafeAreaView className="flex-1 bg-background" edges={['top', 'bottom']} style={RTL_CONTAINER_STYLE}>
        ...
        <View className="flex-1 items-center justify-center px-5">
          <Text className="text-sm text-slate-400" style={{ fontFamily: cairo.medium, ...RTL_TEXT_STYLE }}>
            {t('members.notFound')}
          </Text>
        </View>
      </ScreenSafeAreaView>
  ...
  return (
    <ScreenSafeAreaView className="flex-1 bg-background" edges={['top', 'bottom']} style={RTL_CONTAINER_STYLE}>
      ...
      <ScrollView
        ...
        <MemberHeroCard member={member} onQrPress={() => qrRef.current?.open()} />
        <View className="gap-4">
          ...
          <View className="gap-2 rounded-[8px] border border-slate-100 bg-white p-3">
            {member.overview.map((row) => (
              <MemberOverviewRowView key={`${row.labelKey}-${row.valueKey ?? row.value}`} row={row} />
            ))}
          </View>
        </View>
        <MemberSanctionsList sanctions={member.sanctions} />
      </ScrollView>
      <MemberQrBottomSheet ref={qrRef} federationId={member.federationId} category={member.category} />
    </ScreenSafeAreaView>
  );
}
