...
export default function ParticipantCard({ participant }: ParticipantCardProps) {
  ...
  return (
    <View className="flex-row items-center gap-3 rounded-lg border border-slate-100 bg-white p-3" style={RTL_CONTAINER_STYLE}>
      ...
      <View className="min-w-0 flex-1 gap-1.5">
        <View className="flex-row flex-wrap items-center gap-1.5" style={RTL_CONTAINER_STYLE}>
          ...
          <View
            className={`h-[13px] items-center justify-center rounded-3xl px-1.5 ${
              participant.status === 'suspended' ? 'bg-rejected/10' : 'bg-primary/10'
            }`}>
            <Text
        ...
        <View className="flex-row flex-wrap gap-1.5" style={RTL_CONTAINER_STYLE}>
          <MetaChip label={t('competitions.fields.rating')} value={String(participant.rating)} />
          <MetaChip label={t('competitions.fields.weapon')} value={t(participant.weaponKey)} />
          <MetaChip label={t('competitions.fields.level')} value={participant.level} />
        </View>
      </View>
    </View>
  );
...
function MetaChip({ label, value }: { label: string; value: string }) {
  return (
    <View className="flex-row items-center gap-1 rounded-md bg-slate-50 px-1.5 py-1" style={RTL_CONTAINER_STYLE}>
      <Text className="text-[10px] text-slate-400" style={{ fontFamily: cairo.regular }}>
        {label}
      </Text>
      <View className="size-0.5 rounded-full bg-slate-300" />
      <Text className="text-[10px] text-label" style={{ fontFamily: cairo.medium }}>
        {value}
      </Text>
    </View>
  );
